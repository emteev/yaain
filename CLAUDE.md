...
**Owner:** Hot Stacks (Em)  
**Purpose:** Daily pipeline that crawls defined sources, filters for Claude-family content using Claude Sonnet 4.6, and writes a valid RSS 2.0 feed to `feed.xml`.  
**Automation:** GitHub Actions runs `yaain/main.py` daily at noon UTC. On success it commits `feed.xml` and `yaain/seen.json` back to the repo.
...
YAAIN/
  ...
  .github/
    workflows/
      daily.yml   GitHub Actions cron definition.
  feed.xml        Generated output. Do not edit manually.
```
...
```
GitHub Actions (noon UTC)
  → main.py
    → fetch_all(SOURCES)           # fetcher.py — returns list of raw item dicts
    → filter_new(items, seen)      # state.py — drops URLs already in seen.json
    → filter_items(new, api_key)   # filter.py — Claude API, drops non-Claude content
    → load_existing_items(path)    # feed.py — parses current feed.xml
    → build_feed(passed, existing) # feed.py — merges and writes feed.xml
    → mark_seen + save             # state.py — updates seen.json
  → Actions commits feed.xml + seen.json
```
...
Generic scraper for Anthropic pages. Extracts linked items from article/heading/list elements. Skips links with text under 10 chars. Takes up to 30 distinct URLs per page.
...
**`build_feed(new_items, existing_items, feed_path) → int`**  
Deduplicates new items against existing by URL (used as guid). Prepends passing new items to front of list. Trims to 100 items. Writes pretty-printed RSS 2.0 XML to `feed_path`. Returns count of items added.
...
- `feed.xml` is a raw file in the repo, not served via GitHub Pages. Access via the raw GitHub URL.
